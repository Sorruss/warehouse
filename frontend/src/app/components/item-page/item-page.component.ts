...
export class ItemPageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.doesUserHavePrivileges =
      this.user.user_role === 'admin' || this.user.user_role === 'moder';
  }
  ngOnDestroy(): void {
  ...
  retrieveItem(): void {
    this.itemsService.get(this.id).subscribe({
      next: (data) => {
        this.item = data;
        this.directAccessGuard.guard(this.item.company_id);
        console.log(data);
      },
      error: (error) => {
        console.log(error);
  ...
  removeItem(): void {
    if (!this.doesUserHavePrivileges) {
      return;
    }

    // Deleting item's photo
    this.itemsService.deletePhotoById(this.id).subscribe({
      next: (data) => {
